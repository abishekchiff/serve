...
class TransformersSeqClassifierHandler(BaseHandler, ABC):
    ...
    def preprocess(self, data):
        ...
        input_text = None
        inp = data
        print("Bert preprocess data", inp)
        if inp is not None:
    ...
    def inference(self, inputs):
        ...
        if self.setup_config["mode"]== "sequence_classification":
            predictions_result = self.model(input_ids)

            logger.info("Model predicted BEFORE ARGMAX: '%s'", predictions_result)
            predictions_result = predictions_result.argmax(1).item()
            logger.info("Model predicted: '%s'", predictions_result)
        ...
        # Handling inference for token_classification.
            ...
            predictions_result = torch.argmax(outputs, dim=2)
            logger.info("Model predicted: '%s'", predictions_result)

        return predictions_result, inputs

    def postprocess(self, pred_result, inp):

        # TODO: Add any needed post-processing of the model predictions here
        if isinstance(pred_result, tuple):
            answer_start_scores, answer_end_scores = pred_result

        if self.setup_config["mode"]== "sequence_classification":
            if self.mapping:
                prediction = self.mapping[str(pred_result)]
        elif self.setup_config["mode"]== "question_answering":
            answer_start = torch.argmax(answer_start_scores)  # Get the most likely beginning of answer with the argmax of the score
            answer_end = torch.argmax(answer_end_scores) + 1  # Get the most likely end of answer with the argmax of the score
            input_ids = inp["input_ids"].tolist()
            ...
            tokens = self.tokenizer.tokenize(self.tokenizer.decode(inp["input_ids"]))
            if self.mapping:
                label_list = self.mapping["label_list"]
            label_list = label_list.strip('][').split(', ')
            prediction = [(token, label_list[prediction]) for token, prediction in zip(tokens, pred_result.tolist())]
